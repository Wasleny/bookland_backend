...
class BookSchema(BaseModel):
    ...
    series_id: str = Field(..., description="ID da série do livro")
    original_series_id: str = Field(..., description="ID da série original do livro")
    book_number: float = Field(..., description="Volume dentro da série do livro")
    ...
    sinopsis: str = Field(..., description="Sinopse do livro")
    ...
    asin: str = Field(..., min_length=10, max_length=10, description="ASIN do livro")
    language: str = Field(..., description="Idioma do livro")
    alternative_edition_ids: list = Field(
...
class BookResponseSchema(BaseModel):
    ...
    main_genre: str = Field(..., description="Gênero principal do livro")
    ...
    cover: str = Field(..., description="URL da capa do livro")
    series: str = Field(..., description="Série do livro")
    original_series: str = Field(..., description="Série original do livro")
    book_number: float = Field(..., description="Volume dentro da série do livro")
    average_rating: float = Field(..., description="Avaliação média do livro")
    reviews_count: float = Field(..., description="Total de resenhas do livro")
    ratings_count: float = Field(..., description="Total de avaliações do livro")
    ...
    publication_date: str = Field(..., description="Data de publicação do livro")
    publisher: str = Field(..., description="Editora que publicou a obra")
    isbn10: str = Field(..., description="ISBN-10 do livro")
    isbn13: str = Field(..., description="ISBN-13 do livro")
    asin: str = Field(..., description="ASIN do livro")
    language: str = Field(..., description="Idioma do livro")
    alternative_edition_ids: list = Field(
        ..., description="Lista de IDs das obras alternativas (outras edições)"
    )
    ...
    def from_entity(cls, book):
        return cls(
            ...
            main_genre={"id": book.main_genre.id, "name": book.main_genre.name.value},
            secondary_genres=[
